...
cp -r neoma-argos neoma-<your-package-name>
...
- `argos` → Your package name (e.g., "garmr", "validation")
- `NestJS audit trails — who changed what, when, and what it looked like.` → Short description
- `https://github.com/neomaventures/argos` → GitHub repository URL (e.g., "https://github.com/shipdventures/neoma-garmr")

**Quick find/replace:**
```bash
# macOS/Linux
find . -type f -name "*.json" -o -name "*.md" -o -name "*.ts" | xargs sed -i '' 's/argos/your-package-name/g'
find . -type f -name "*.json" -o -name "*.md" -o -name "*.ts" | xargs sed -i '' 's/NestJS audit trails — who changed what, when, and what it looked like./Your description/g'
find . -type f -name "*.json" -o -name "*.md" -o -name "*.ts" | xargs sed -i '' 's|https://github.com/neomaventures/argos|https://github.com/your-org/your-repo|g'
```

### 3. Rename directories

```bash
mv libs/argos libs/your-package-name
```
...
**Example**: The template includes `libs/argos/src/modules/example.module.spec.ts` showing how to test NestJS modules.

### Complete Testing Flow

1. **Write library code** in `libs/argos/src/`
2. **Write unit tests** alongside your code (`*.spec.ts`)
3. **Export from** `libs/argos/src/index.ts`
